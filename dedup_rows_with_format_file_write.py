...
file_path = 'file_write_output_json_file.json'
file_path_2 = 'file_write_output_json_file_2.json'
output_path_1 = 'file_write_dedup_json_file.json'
output_path_2 = 'file_write_re_format_file.json'
...
for elem in unique_data:
    ...
    result_dict[elem['fullProcessPath']][elem['filePath']] = elem['filePathUnderProcessCount']
result_list = []
...
#   tmp['largest_25_total_count'] = sum(d['writeCount'] for d in tmp['written_files'][:25])
    tmp['written_files'] = tmp['written_files'][:reportTopN]
...
with open(output_path_2, "w") as json_file:
    json.dump(sorted_result_list[:eventThreshold], json_file, indent=4)
