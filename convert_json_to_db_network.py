...
db_file_path = 'network_events_database.db'
...
cursor.execute('''
CREATE TABLE IF NOT EXISTS networkConnectionEvents (
    processPath TEXT,
    timestamp TEXT
)
''')
...
for record in data:
    if record['eventType1'] != 'ipv4NetworkEvent':
        continue;
    if 'processPath' in record and 'process' in record: 
        if record['processPath'] is not None and record['process'] is not None:
            cursor.execute('''INSERT INTO networkConnectionEvents (processPath, timestamp) VALUES (?, ?)''', (record['processPath'] + "\\" + record['process'], record['timestamp1']))
        else:
            cursor.execute('''INSERT INTO networkConnectionEvents (processPath, timestamp) VALUES (?, ?)''', ('System', record['timestamp1']))
