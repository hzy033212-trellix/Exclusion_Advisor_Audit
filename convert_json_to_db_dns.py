...
db_file_path = 'dns_events_database.db'
output_json_file = 'dns_output_json_file.json'
output_json_file_2 = 'dns_output_json_file_2.json'
...
cursor.execute('''
CREATE TABLE IF NOT EXISTS dnsEvents (
    fullPath TEXT,
    processPath TEXT,
    process TEXT,
    timestamp TEXT
...
for record in data:
    if record['eventType1'] != 'dnsLookupEvent':
        continue;
    cursor.execute('''INSERT INTO dnsEvents (fullPath, processPath, process, timestamp) VALUES (?, ?, ?, ?)''', (record['hostname'], record['processPath'], record['process'], record['timestamp1']))
