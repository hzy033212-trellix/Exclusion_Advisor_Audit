...
db_file_path = 'image_load_events_database.db'
output_json_file = 'image_load_output_json_file.json'
output_json_file_2 = 'image_load_output_json_file_2.json'
...
cursor.execute('''
CREATE TABLE IF NOT EXISTS imageLoadEvents (
    fullPath TEXT,
    processPath TEXT,
    process TEXT,
    timestamp TEXT
...
for record in data:
    if record['eventType1'] != 'imageLoadEvent':
        continue;
    try:
        cursor.execute('''INSERT INTO imageLoadEvents (fullPath, processPath, process, timestamp) VALUES (?, ?, ?, ?)''', (record['fullPath'], record['processPath'], record['process'], record['timestamp1']))
...
SELECT 
    ...
    COUNT(*) OVER(PARTITION BY CONCAT(processPath, "\\", process), fullPath) AS filePathUnderProcessCount
FROM 
    imageLoadEvents;
'''
