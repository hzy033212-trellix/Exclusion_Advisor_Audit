...
db_file_path = 'reg_key_events_database.db'
output_json_file = 'reg_key_output_json_file.json'
output_json_file_2 = 'reg_key_output_json_file_2.json'
...
cursor.execute('''
CREATE TABLE IF NOT EXISTS regKeyEvents (
    fullPath TEXT,
    processPath TEXT,
    process TEXT,
    timestamp TEXT
...
for record in data:
    if record['eventType1'] != 'regKeyEvent':
        continue;
    cursor.execute('''INSERT INTO regKeyEvents (fullPath, processPath, process, timestamp) VALUES (?, ?, ?, ?)''', (record['keyPath'], record['processPath'], record['process'], record['timestamp1']))
