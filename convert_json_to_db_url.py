...
db_file_path = 'url_events_database.db'
...
cursor.execute('''
CREATE TABLE IF NOT EXISTS urlMonitorEvents (
    processPath TEXT,
    timestamp TEXT
)
''')
...
for record in data:
    if record['eventType1'] != 'urlMonitorEvent':
        continue;
    if 'processPath' in record and 'process' in record: 
        if record['processPath'] is not None and record['process'] is not None:
            cursor.execute('''INSERT INTO urlMonitorEvents (processPath, timestamp) VALUES (?, ?)''', (record['processPath'] + "\\" + record['process'], record['timestamp1']))
        else:
            cursor.execute('''INSERT INTO urlMonitorEvents (processPath, timestamp) VALUES (?, ?)''', ('System', record['timestamp1']))
